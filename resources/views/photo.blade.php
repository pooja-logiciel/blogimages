	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-lg-4 col-sm-6">
				<div class="thumbnail">
                    <span><img src="{{asset('storage/'.$row['user']['path'])}}" style="width:50px; height:50px; border-radius:50px; margin:1px 5px 3px 1px;"  />
                        <a href="demo/{{$row['user']['id']}}"><b>@php print_r($row['user']['name']); @endphp </b></a>
                    </span>
					<img src="{{asset('storage/'.$row['path'])}}" style="width:300px; height:300px "/>
                    <a href="view/{{$row['id']}}"><span class="material-symbols-outlined" style="margin: 0px 0px 0px 10px;">visibility</span></a>
                    <b>{{$row['view']}}</b>
                    <a href="favt/{{$row['id']}}"><span class="material-symbols-outlined" style="margin: 10px 0px 0px 250px;">heart_plus</span></a>
				</div>
			</div>
            @endforeach
	    </div>
