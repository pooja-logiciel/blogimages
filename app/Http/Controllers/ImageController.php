...
class ImageController extends Controller
{
    ...
    public function demo($id)
    {
        ...
        $following=follower::where('following',$id)->count('user_id');
        $follower=follower::where('user_id',$id)->count('following');
        if(follower::where('user_id','=',$id)->where('following','=',$user_id)->first()==Null)
        {
          $check=0;
        }
        else
        ...
        return view('user1',compact('user','data','follower','check','following'));
    ...
    public function follower($followerid){
        $user=auth()->user()->id;
        if(follower::where('following','=',$user)->Where('user_id','=',$followerid)->first()==Null)
        {
            $user=auth()->user()->id;
            $data=new follower;
    ...
    public function Updatefollower($id){
        $user=auth()->user()->id;
        follower::where('following','=',$user)->where('user_id','=',$id)->first()->delete();
        return back();
    }
